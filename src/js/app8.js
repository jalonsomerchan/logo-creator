...
function loadState() {
  try {
    const raw = localStorage.getItem(STORE) || localStorage.getItem('logo-creator-v4') || localStorage.getItem('logo-creator-v3');
    ...
    // Start with an empty project if stored data is invalid.
  }
...
function updateCursor() {
  const canvas = document.querySelector('#canvasWrap');
  if (panDrag) canvas.style.cursor = 'grabbing';
  else if (moveDrag) canvas.style.cursor = 'move';
  else if (state.tool === 'pan') canvas.style.cursor = 'grab';
  else if (state.tool === 'select') canvas.style.cursor = 'default';
  else if (state.mode === 'cutout') canvas.style.cursor = 'cell';
...
function onPointerDown(event) {
  ...
  if (state.tool === 'pan' || event.button === 1) { panDrag = { screenPoint, camera: clone(state.camera) }; render(); return; }
...
function addPoint(point) {
  ...
  if (first && samePoint(first, point) && state.points.length >= 3) { closePolygon(); return; }
...
function startMove(screenPoint, additive) {
  ...
  if (single && vertexIndex >= 0) { remember(); moveDrag = { type: 'vertex', id: single.id, vertexIndex, changed: false }; render(); return; }
...
function moveByKey(key, shift, fine) {
  ...
  if (shift || !state.selectedIds.length) { state.camera.x -= delta.x * state.camera.zoom; state.camera.y -= delta.y * state.camera.zoom; render(); return; }
  remember();
...
function changeStyle(key, value) {
  ...
  if (shapes.length) { remember(); shapes.forEach((shape) => { shape[key] = value; }); }
  state[key] = value;
  saveAndRender();
}
...
function undo() {
  if (state.points.length) { state.points.pop(); render(); return; }
...
function newProject() {
  ...
  remember();
  state = { ...clone(defaults), camera: clone(defaults.camera), panelCollapsed: state.panelCollapsed };
  clipboard = [];
  hoverPoint = null;
  saveAndRender();
}
