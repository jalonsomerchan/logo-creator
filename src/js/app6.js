...
function loadState() {
  try {
    ...
    if (stored?.shapes) return { ...copy(defaultState), ...stored, points: [], camera: copy(defaultState.camera) };
  } catch {
    // Empty fallback.
  }
...
function remember() {
  undoStack.push(copy({ ...state, points: [] }));
  ...
  app.innerHTML = `
    <div class="top-left">
      <section class="floating-card brand-card">
        ...
        <div>
          ...
          <p class="brand-subtitle">Une intersecciones de la retícula para cerrar figuras</p>
        </div>
      </section>
    </div>
    ...
    <aside class="panel right-panel" id="panel">
      <div class="toolbar-header">
        <div class="toolbar-title"><strong>Herramientas</strong><span>Polígonos cerrados sobre retícula</span></div>
      ...
      <div class="toolbar-body">
        ...
        <section class="panel-section"><h2>Retícula</h2>
          ...
          <p class="empty-state">Los puntos válidos son solo las intersecciones: esquinas y centros donde cruzan las diagonales.</p>
        </section>
        <section class="panel-section"><h2>Capas</h2><div class="layer-list" id="shapeList"></div>
        ...
        <section class="panel-section"><h2>Exportar</h2><div class="export-row"><button class="export-button primary" data-export="svg" type="button">${icon('filetype-svg')} SVG</button><button class="export-button" data-export="png" type="button">${icon('filetype-png')} PNG</button></div><div class="status-line"><span id="pointStatus"></span><span id="shapeStatus"></span></div></section>
        <section class="panel-section"><h2>Atajos</h2><div class="dashed-help help-grid"><span><strong>Enter</strong> cerrar figura</span><span><strong>Esc</strong> cancelar puntos</span><span><strong>Flechas</strong> mover selección</span><span><strong>Shift + flechas</strong> mover vista</span></div></section>
      </div>
    </aside>
...
function bindEvents() {
  ...
  canvas.addEventListener('dblclick', closeCurrentPolygon);
...
function snapPointer(event) {
  return snapToIntersection(screenToWorld(pointerPosition(event)));
}
...
function renderGrid(rect) {
  ...
  const cornerPoints = intersections.filter((point) => isMultiple(point.x, size) && isMultiple(point.y, size));
  ...
  verticals.forEach((x) => gridLayer.append(svgNode('line', { x1: x, y1: 0, x2: x, y2: rect.height, stroke: 'rgba(30,29,27,.12)' })));
  horizontals.forEach((y) => gridLayer.append(svgNode('line', { x1: 0, y1: y, x2: rect.width, y2: y, stroke: 'rgba(30,29,27,.12)' })));

  const cells = cornerPoints;
  cells.forEach((point) => {
  ...
  intersections.forEach((point) => {
    ...
    gridLayer.append(svgNode('circle', { cx: screen.x, cy: screen.y, r: 2.25, fill: 'rgba(30,29,27,.32)' }));
  });
  if (hoverPoint) {
    const screen = worldToScreen(hoverPoint);
    gridLayer.append(svgNode('circle', { cx: screen.x, cy: screen.y, r: 7, fill: '#fff', stroke: '#111', 'stroke-width': 2 }));
  }
}
...
function renderShapes() {
  ...
  state.shapes.forEach((shape) => {
    ...
    if (shape.id === state.selectedId) renderSelection(shape);
  });
}
...
function renderSelection(shape) {
  ...
  shape.points.forEach((point) => {
    const screen = worldToScreen(point);
    shapeLayer.append(svgNode('circle', { cx: screen.x, cy: screen.y, r: 5.5, fill: '#fff', stroke: '#111', 'stroke-width': 1.7 }));
  });
}

function renderDraft() {
  clear(draftLayer);
  const livePoints = [...state.points];
  if (hoverPoint && state.tool === 'draw' && livePoints.length) livePoints.push(hoverPoint);
  if (livePoints.length) {
    draftLayer.append(svgNode('polyline', { points: pointList(livePoints), fill: 'none', stroke: state.mode === 'cutout' ? '#dc2626' : state.fill, 'stroke-width': Math.max(2, state.strokeWidth || 2), 'stroke-dasharray': state.mode === 'cutout' ? '8 5' : 'none', 'stroke-linejoin': 'round', 'stroke-linecap': 'round' }));
  }
  state.points.forEach((point, index) => {
    const screen = worldToScreen(point);
    draftLayer.append(svgNode('circle', { cx: screen.x, cy: screen.y, r: index ? 4 : 6, fill: index ? '#111' : '#ffd12e', stroke: '#fff', 'stroke-width': 2 }));
  });
}
...
function renderPanel() {
  ...
  document.querySelector('#shapeStatus').textContent = `${state.shapes.filter((shape) => shape.visible).length} figuras`;
  const list = document.querySelector('#shapeList');
  clear(list);
  if (!state.shapes.length) {
    list.innerHTML = '<p class="empty-state">Empieza uniendo puntos hasta cerrar una figura.</p>';
    return;
  }
  [...state.shapes].reverse().forEach((shape) => {
    const item = document.createElement('button');
    item.type = 'button';
    item.className = `layer-item ${shape.id === state.selectedId ? 'is-active' : ''}`;
    ...
    item.addEventListener('click', () => selectShape(shape.id));
    list.append(item);
  });
}
...
function onPointerDown(event) {
  ...
  if (state.tool === 'pan' || event.shiftKey || event.button === 1) {
    panDrag = { screenPoint, camera: copy(state.camera) };
    return;
  }
  if (state.tool === 'select') {
    startMove(screenPoint);
    return;
  }
...
function onPointerUp(event) {
  ...
  if (moveDrag?.changed) saveAndRender();
  panDrag = null;
  moveDrag = null;
}
...
function closeCurrentPolygon() {
  ...
  remember();
  if (state.mode === 'cutout') addCutout(state.points);
  else {
    const shape = { id: `shape-${++id}`, name: `Figura ${state.shapes.length + 1}`, points: copy(state.points), holes: [], fill: state.fill, stroke: state.stroke, strokeWidth: state.strokeWidth, opacity: state.opacity, visible: true };
...
function addCutout(points) {
  ...
  for (let index = state.shapes.length - 1; index >= 0; index -= 1) {
    ...
    if (intersects) {
      shape.holes = [...(shape.holes || []), copy(points)];
      state.selectedId = shape.id;
...
function startMove(screenPoint) {
  const worldPoint = screenToWorld(screenPoint);
  const current = selectedShape();
  const vertexIndex = current ? current.points.findIndex((point) => distance(worldPoint, point) * state.camera.zoom <= 12) : -1;
  if (current && vertexIndex >= 0) {
    remember();
    moveDrag = { type: 'vertex', id: current.id, vertexIndex, changed: false };
    return;
  }
  ...
  selectShape(hit.id);
  remember();
  moveDrag = { type: 'shape', id: hit.id, start: snapToIntersection(worldPoint), points: copy(hit.points), holes: copy(hit.holes || []), changed: false };
}

function moveSelectedByPointer(event) {
  const shape = selectedShape();
  if (!shape || shape.id !== moveDrag.id) return;
  const point = snapPointer(event);
  if (moveDrag.type === 'vertex') shape.points[moveDrag.vertexIndex] = point;
  else {
    const delta = { x: point.x - moveDrag.start.x, y: point.y - moveDrag.start.y };
    shape.points = moveDrag.points.map((source) => snapToIntersection({ x: source.x + delta.x, y: source.y + delta.y }));
    shape.holes = moveDrag.holes.map((hole) => hole.map((source) => snapToIntersection({ x: source.x + delta.x, y: source.y + delta.y })));
  }
...
function onKeyDown(event) {
  ...
  if (command && key === 'c') { event.preventDefault(); copyShape(); return; }
  if (command && key === 'v') { event.preventDefault(); pasteShape(); return; }
  if (event.key.startsWith('Arrow')) { event.preventDefault(); moveByKey(event.key, event.shiftKey, event.altKey); return; }
  if (key === 'enter') closeCurrentPolygon();
...
function runAction(action) {
  ...
  if (action === 'duplicate') duplicateShape();
}

function selectedShape() { return state.shapes.find((shape) => shape.id === state.selectedId); }
function selectShape(idValue) { const shape = state.shapes.find((item) => item.id === idValue); if (!shape) return; state.selectedId = shape.id; state.fill = shape.fill; state.stroke = shape.stroke; state.strokeWidth = shape.strokeWidth; state.opacity = shape.opacity; saveAndRender(); }
function changeStyle(key, value) { const shape = selectedShape(); if (shape) { remember(); shape[key] = value; } state[key] = value; saveAndRender(); }
function moveShape(delta) { const shape = selectedShape(); if (!shape) return; shape.points = shape.points.map((point) => snapToIntersection({ x: point.x + delta.x, y: point.y + delta.y })); shape.holes = (shape.holes || []).map((hole) => hole.map((point) => snapToIntersection({ x: point.x + delta.x, y: point.y + delta.y }))); }
function deleteSelected() { if (!selectedShape()) return; remember(); state.shapes = state.shapes.filter((shape) => shape.id !== state.selectedId); state.selectedId = state.shapes.at(-1)?.id || null; saveAndRender(); }
function duplicateShape() { const shape = selectedShape(); if (!shape) return; savedLayer = copy(shape); pasteShape(); }
function copyShape() { const shape = selectedShape(); if (!shape) return; savedLayer = copy(shape); showToast('Figura copiada'); }
function pasteShape() { if (!savedLayer) return; remember(); const shape = copy(savedLayer); shape.id = `shape-${++id}`; shape.name = `${savedLayer.name} copia`; shape.points = shape.points.map((point) => snapToIntersection({ x: point.x + state.gridSize, y: point.y + state.gridSize })); shape.holes = (shape.holes || []).map((hole) => hole.map((point) => snapToIntersection({ x: point.x + state.gridSize, y: point.y + state.gridSize }))); state.shapes.push(shape); state.selectedId = shape.id; saveAndRender(); }
function undo() { if (!undoStack.length) return; redoStack.push(copy({ ...state, points: [] })); restore(undoStack.pop()); }
function redo() { if (!redoStack.length) return; undoStack.push(copy({ ...state, points: [] })); restore(redoStack.pop()); }
...
function exportLogo(type) {
  const shapes = state.shapes.filter((shape) => shape.visible && shape.points.length >= 3);
  if (!shapes.length) { showToast('Dibuja al menos una figura'); return; }
  ...
  const body = shapes.map((shape) => { const moved = { ...shape, points: shiftPoints(shape.points, box), holes: (shape.holes || []).map((hole) => shiftPoints(hole, box)) }; return `<path d="${shapePath(moved, false)}" fill="${shape.fill}" fill-rule="evenodd" stroke="${shape.stroke}" stroke-width="${shape.strokeWidth}" opacity="${shape.opacity}"/>`; }).join('\n  ');
  ...
  image.onload = () => { const scale = Math.max(2, Math.min(4, 1024 / Math.max(width, height))); const canvas = document.createElement('canvas'); canvas.width = Math.ceil(width * scale); canvas.height = Math.ceil(height * scale); canvas.getContext('2d').drawImage(image, 0, 0, canvas.width, canvas.height); canvas.toBlob((blob) => { if (blob) download('logo-creator.png', blob); URL.revokeObjectURL(url); }, 'image/png'); };
  image.src = url;
}
